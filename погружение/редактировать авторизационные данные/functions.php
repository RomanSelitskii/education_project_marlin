...
/*
    ...
    Description: редактирует авторизационные данные пользователя
...
function edit_credentials($user_id, $email, $password){
    ...
    //если адрес почты свободен, то обновляем данные в базе
    if($email_status == TRUE){
        $sql = "UPDATE users_table SET email = :email, password = :password  WHERE id = :user_id";
        $statement = $pdo->prepare($sql);
        $statement->execute([
            'user_id' => $user_id,
            'email' => $email,
